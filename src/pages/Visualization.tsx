...
const Visualization = () => {
  ...
  const handleExportCSV = () => {
    // Example: Export comparisonData as CSV
  ...
  const handleExportPDF = () => {
    // Example: Export chart summary as PDF
    const doc = new jsPDF();
    doc.text("Battery Data Visualization Report", 10, 10);
    doc.text("Summary:", 10, 20);
    doc.text(`Active Tab: ${activeTab}`, 10, 30);
    doc.text("Comparison Data (first 3 rows):", 10, 40);
    comparisonData.slice(0, 3).forEach((row, idx) => {
      doc.text(JSON.stringify(row), 10, 50 + idx * 10);
    });
