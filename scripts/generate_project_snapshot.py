        ...
        for file in files:
            if file.endswith(('.ts', '.tsx')) and not file.endswith(('.d.ts', '.test.ts', '.spec.ts')):
                ...
                if any(len(file_info.get(k, [])) > 0 for k in ["functions", "classes", "interfaces", "components", "hooks", "decorators"]):
                    ...
                    # 若檔案名稱符合模組命名慣例（如以Module結尾），也記錄
                    if file.endswith('Module.ts') or file.endswith('Module.tsx'):
                        project_info.append({
                            "path": str(file_path),
                            "classes": [],
                            "functions": [],
                            ...
                            "hooks": [],
                            "decorators": [],
                            "note": "Empty module file detected, please check manually."
                        })
